...
// after they return.
type UsabilityOracle func(balancer.Conns, func(balancer.Conn) HealthState) []balancer.Conn
...
//
//	HealthyState, UnknownState, DegradedState, UnhealthyState
func NewOracle(threshold HealthState, opts ...NewOracleOption) UsabilityOracle {
	...
	if !options.preferHealthier {
		...
		return func(allConns balancer.Conns, state func(balancer.Conn) HealthState) []balancer.Conn {
			length := allConns.Len()
			usable := make([]balancer.Conn, 0, length)
			for i := 0; i < length; i++ {
	...
	return func(allConns balancer.Conns, state func(balancer.Conn) HealthState) []balancer.Conn {
		length := allConns.Len()
		connsByState := map[HealthState][]balancer.Conn{}
		for i := 0; i < length; i++ {
			connection := allConns.Get(i)
...
// if this were set to 3 and the oracle's threshold were UnknownState, but there were
// only 2 HealthyState connections, one UnknownState connection will be randomly selected
...
// the threshold were UnknownState and there is only 1 healthy connection, one UnknownState
