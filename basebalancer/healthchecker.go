...
var (
	// NoOpHealthChecker is a checker implementation that does nothing. It never updates
	// the health states of any connections and uses no resources.
	NoOpHealthChecker HealthChecker = noOpHealthChecker{}
)

// HealthChecker manages health checks. It creates new checking processes as new
...
type HealthChecker interface {
	...
	New(context.Context, balancer.Conn, HealthTracker) io.Closer
}

// HealthTracker represents an object that tracks the health state of various connections.
// This is the interface through which a HealthChecker communicates state updates.
type HealthTracker interface {
	UpdateHealthState(balancer.Conn, HealthState)
}

type noOpHealthChecker struct{}

func (n noOpHealthChecker) New(_ context.Context, _ balancer.Conn, _ HealthTracker) io.Closer {
	return noOpCloser{}
}
