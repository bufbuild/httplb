...
// * No subsetting. Conns are created for all resolved addresses.
// * No health checks. All connections are considered usable.
// * Round-robin picker.
func NewFactory(opts ...NewFactoryOption) balancer.Factory {
	factory := &baseBalancerFactory{}
...
// NewFactoryOption is an option for configuring the behavior of a ConnManagerFactory
// return from NewFactory.
type NewFactoryOption interface {
	apply(balancer *baseBalancerFactory)
}

// WithConnManager configures a balancer factory to use the given
// connmanager.ConnManagerFactory. It will be used to create a connmanager.ConnManager
// for each Balancer. The ConnManager is what decides what connections to
// create or remove.
func WithConnManager(connManager ConnManagerFactory) NewFactoryOption {
	return newFactoryOptionFunc(func(factory *baseBalancerFactory) {
...
func WithPicker(picker PickerFactory) NewFactoryOption {
	return newFactoryOptionFunc(func(factory *baseBalancerFactory) {
...
// connections (which remain in the UnknownState health state) will be considered
// usable.
func WithHealthChecks(checker HealthChecker, oracle UsabilityOracle) NewFactoryOption {
	return newFactoryOptionFunc(func(factory *baseBalancerFactory) {
...
//
//	balancer.WithWarmDefinition(1, 0, healthchecker.UnknownState)
//
// So there must be at least one connection, and it must be either in the
// UnknownState or HealthyState state.
//
...
func WithWarmDefinition(numConns int, percentConns float64, state HealthState) NewFactoryOption {
	return newFactoryOptionFunc(func(factory *baseBalancerFactory) {
...
type newFactoryOptionFunc func(factory *baseBalancerFactory)

func (o newFactoryOptionFunc) apply(factory *baseBalancerFactory) {
	o(factory)
}

type baseBalancerFactory struct {
	connManager     ConnManagerFactory
	picker          PickerFactory
	healthChecker   HealthChecker
	usabilityOracle UsabilityOracle
	warmMinCount    int
	warmMinPercent  float64
	warmMaxState    HealthState
}

func (f *baseBalancerFactory) applyDefaults() {
	if f.connManager == nil {
		f.connManager = NewConnManagerFactory()
	}
	if f.picker == nil {
		f.picker = RoundRobinPickerFactory
	}
	if f.healthChecker == nil {
		f.healthChecker = NoOpHealthChecker
	}
	if f.usabilityOracle == nil {
		f.usabilityOracle = NewOracle(UnknownState)
	}
}

func (f *baseBalancerFactory) New(ctx context.Context, scheme, hostPort string, pool balancer.ConnPool) balancer.Balancer {
	...
	balancer := &baseBalancer{
		...
		connInfo:        map[balancer.Conn]connInfo{},
	}
...
type baseBalancer struct {
	...
	pool            balancer.ConnPool
	connManager     ConnManager     // +checklocksignore: mu is not required, but happens to always be held.
	picker          PickerFactory   // +checklocksignore: mu is not required, but happens to always be held.
	healthChecker   HealthChecker   // +checklocksignore: mu is not required, but happens to always be held.
	usabilityOracle UsabilityOracle // +checklocksignore: mu is not required, but happens to always be held.
	...
	warmMaxState    HealthState     // +checklocksignore: mu is not required, but happens to always be held.
	...
	mu sync.Mutex
	// +checklocks:mu
	latestPicker balancer.Picker
	// +checklocks:mu
	latestUsableConns balancer.ConnSet
	// +checklocks:mu
	conns []balancer.Conn
	// +checklocks:mu
	connInfo map[balancer.Conn]connInfo
...
func (b *baseBalancer) UpdateHealthState(connection balancer.Conn, state HealthState) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *baseBalancer) warmedUp(connection balancer.Conn) {
	b.mu.Lock()
	defer b.mu.Unlock()
	info, ok := b.connInfo[connection]
	if !ok {
...
func (b *baseBalancer) OnResolve(addresses []resolver.Address) {
...
func (b *baseBalancer) OnResolveError(err error) {
...
func (b *baseBalancer) Close() error {
	b.cancel()
...
func (b *baseBalancer) receiveAddrs(ctx context.Context) {
	defer func() {
...
func (b *baseBalancer) updateConns(newAddrs []resolver.Address, removeConns []balancer.Conn) []balancer.Conn {
	numAdded := len(newAddrs)
	numRemoved := len(removeConns)
	addConns := make([]balancer.Conn, 0, numAdded)
	...
	setToRemove := make(map[balancer.Conn]struct{}, numRemoved)
	...
	b.mu.Lock()
	defer b.mu.Unlock()
	newConns := make([]balancer.Conn, 0, len(b.conns)+numAdded-numRemoved)
...
// +checklocks:b.mu
func (b *baseBalancer) connHealthLocked(c balancer.Conn) HealthState {
	return b.connInfo[c].state
}

// +checklocks:b.mu
func (b *baseBalancer) newPickerLocked() {
	usable := b.usabilityOracle(balancer.ConnsFromSlice(b.conns), b.connHealthLocked)
	if len(usable) == 0 {
		addrs := b.latestAddrs.Load()
	...
	usableSet := balancer.ConnSetFromSlice(usable)
	if !usableSet.Equals(b.latestUsableConns) {
		// only recreate picker if the connections actually changed
		b.latestPicker = b.picker.New(b.latestPicker, balancer.ConnsFromSlice(usable))
...
// +checklocks:b.mu
func (b *baseBalancer) isWarmLocked(conns []balancer.Conn, allLen int) bool {
...
func (b *baseBalancer) setErrorPicker(err error) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
// +checklocks:b.mu
func (b *baseBalancer) setErrorPickerLocked(err error) {
	b.pool.UpdatePicker(balancer.ErrorPicker(err), false)
}
