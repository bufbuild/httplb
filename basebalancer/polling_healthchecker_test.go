...
func TestPollingChecker(t *testing.T) {
	...
	// UnhealthyState (HTTP error)
	connection := make(fakeConnChan)
	close(connection)
	err := checker.New(ctx, connection, tracker).Close()
	require.NoError(t, err)
	assert.Equal(t, UnhealthyState, <-tracker)

	// UnhealthyState (HTTP 5xx)
...
func (f fakeHealthTracker) UpdateHealthState(_ balancer.Conn, health HealthState) {
	f <- health
}
