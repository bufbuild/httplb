...
// on a fixed interval.
func NewPollingChecker(config PollingCheckerConfig, prober HealthProber) HealthChecker {
...
// A HealthProber is a type that can perform single-shot healthchecks against a
// connection.
type HealthProber interface {
	Probe(ctx context.Context, conn balancer.Conn) HealthState
}
...
func NewSimpleProber(path string) HealthProber {
	return proberFunc(func(ctx context.Context, conn balancer.Conn) HealthState {
		url := url.URL{
			Scheme: conn.Scheme(),
...
func (r *pollingChecker) New(
	...
	// a concern since the initial state (UnknownState) is already considered to be
	// unhealthy.
	...
	go func() {
		...
		for {
			...
			lastState := state
			switch {
			case result == HealthyState && (state == UnhealthyState || state == DegradedState || state == UnknownState):
				counter++
			...
			case state == HealthyState && (result == UnhealthyState || result == DegradedState || result == UnknownState):
				counter++
...
type proberFunc func(ctx context.Context, conn balancer.Conn) HealthState

func (f proberFunc) Probe(ctx context.Context, conn balancer.Conn) HealthState {
	return f(ctx, conn)
}
