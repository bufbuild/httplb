...
type Picker interface {
	Pick(req *http.Request) (conn Conn, whenDone func(), err error)
}

// ErrorPicker returns a picker that always fails with the given error.
func ErrorPicker(err error) Picker {
	return PickerFunc(func(*http.Request) (Conn, func(), error) {
		return nil, nil, err
	})
}

// PickerFunc is a simple function whose signature implements the Picker interface.
type PickerFunc func(*http.Request) (conn Conn, whenDone func(), err error)

func (f PickerFunc) Pick(req *http.Request) (conn Conn, whenDone func(), err error) {
	return f(req)
}
