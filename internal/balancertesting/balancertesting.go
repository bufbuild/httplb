...
var (
	...
	FakePickerFactory basebalancer.PickerFactory = fakePickerFactory{}
)
...
type FakeConn struct {
	...
	prewarm func(balancer.Conn, context.Context) error
}
...
func (c *FakeConn) UpdateAttributes(attributes resolver.Attrs) {
	addr := c.Address()
...
type FakeConnPool struct {
	...
	Prewarm func(balancer.Conn, context.Context) error // +checklocksignore: mu is not required, but happens to always be held.
...
func (p *FakeConnPool) NewConn(address resolver.Address) (balancer.Conn, bool) {
	p.mu.Lock()
	defer p.mu.Unlock()

	if p.active == nil {
		p.active = balancer.ConnSet{}
	}
	p.index++
...
func (p *FakeConnPool) RemoveConn(toRemove balancer.Conn) bool {
	p.mu.Lock()
	defer p.mu.Unlock()
...
func (p *FakeConnPool) Conns() balancer.Conns {
	p.mu.Lock()
	defer p.mu.Unlock()
	return balancer.ConnsFromConnSet(p.active)
}
...
func (p *FakeConnPool) UpdatePicker(picker balancer.Picker, isWarm bool) {
	p.mu.Lock()
	defer p.mu.Unlock()

	var snapshot balancer.ConnSet
...
func (p *FakeConnPool) SnapshotConns() balancer.ConnSet {
	p.mu.Lock()
	defer p.mu.Unlock()
	snapshot := make(balancer.ConnSet, len(p.active))
...
// are updated.
func (p *FakeConnPool) AwaitConnUpdate(ctx context.Context) (balancer.ConnSet, error) {
	select {
	case <-ctx.Done():
		return nil, ctx.Err()
	case <-p.connsUpdate:
...
type PickerState struct {
	...
	IsWarm        bool
	SnapshotConns balancer.ConnSet
}
...
type FakePicker struct {
	Conns balancer.ConnSet
}

// NewFakePicker constructs a new FakePicker with the given connections.
func NewFakePicker(conns balancer.Conns) *FakePicker {
	return &FakePicker{Conns: balancer.ConnSetFromConns(conns)}
}

// Pick implements the picker.Picker interface.
func (p *FakePicker) Pick(_ *http.Request) (conn balancer.Conn, whenDone func(), err error) {
	for c := range p.Conns {
		return c, nil, nil
	}
...
func (f fakePickerFactory) New(_ balancer.Picker, allConns balancer.Conns) balancer.Picker {
	return NewFakePicker(allConns)
}

// ConnHealth is map that tracks the health state of connections.
type ConnHealth map[balancer.Conn]basebalancer.HealthState

func (ch ConnHealth) AsSet() balancer.ConnSet {
	set := make(balancer.ConnSet, len(ch))
	for k := range ch {
		set[k] = struct{}{}
	}
	return set
...
type FakeHealthChecker struct {
	...
	mu              sync.Mutex
	// +checklocks:mu
	initialState basebalancer.HealthState
	// +checklocks:mu
	trackers map[balancer.Conn]basebalancer.HealthTracker
	// +checklocks:mu
	initialized map[balancer.Conn]chan struct{}
	// +checklocks:mu
	conns ConnHealth
}
...
func NewFakeHealthChecker() *FakeHealthChecker {
	return &FakeHealthChecker{
		...
		initialState:    basebalancer.HealthyState,
		trackers:        map[balancer.Conn]basebalancer.HealthTracker{},
		initialized:     map[balancer.Conn]chan struct{}{},
		conns:           ConnHealth{},
	}
...
func (hc *FakeHealthChecker) New(_ context.Context, connection balancer.Conn, tracker basebalancer.HealthTracker) io.Closer {
	hc.mu.Lock()
	defer hc.mu.Unlock()
...
func (hc *FakeHealthChecker) UpdateHealthState(connection balancer.Conn, state basebalancer.HealthState) {
	hc.mu.Lock()
	defer hc.mu.Unlock()
...
func (hc *FakeHealthChecker) SetInitialState(state basebalancer.HealthState) {
	hc.mu.Lock()
	defer hc.mu.Unlock()
...
// initialized.
func (hc *FakeHealthChecker) AwaitConnectionInitialized(ctx context.Context, connection balancer.Conn) (basebalancer.HealthState, error) {
	hc.mu.Lock()
	initializedChan := hc.initialized[connection]
	hc.mu.Unlock()
	if initializedChan == nil {
		return basebalancer.UnhealthyState, nil
	}
	select {
	case <-ctx.Done():
		return basebalancer.UnknownState, ctx.Err()
...
type SwappableUsabilityOracle struct {
	actual atomic.Pointer[basebalancer.UsabilityOracle]
}
...
func (o *SwappableUsabilityOracle) Do(conns balancer.Conns, state func(balancer.Conn) basebalancer.HealthState) []balancer.Conn {
	ptr := o.actual.Load()
	if ptr == nil {
		// no oracle set? just treat everything as usable
		connSlice := make([]balancer.Conn, conns.Len())
...
func (o *SwappableUsabilityOracle) Set(oracle basebalancer.UsabilityOracle) {
...
func DeterministicConnManagerFactory(factory basebalancer.ConnManagerFactory) (result basebalancer.ConnManagerFactory, activeCount func() int) {
	deterministicFactory := &deterministicConnManagerFactory{ConnManagerFactory: factory}
...
type deterministicConnManagerFactory struct {
	basebalancer.ConnManagerFactory
	// +checkatomic
	active atomic.Int32
}

func (f *deterministicConnManagerFactory) New(ctx context.Context, scheme, hostPort string, updateConns basebalancer.ConnUpdater) basebalancer.ConnManager {
	f.active.Add(1)
	connMgr := f.ConnManagerFactory.New(ctx, scheme, hostPort, func(newAddrs []resolver.Address, removeConns []balancer.Conn) (added []balancer.Conn) {
...
func FindConn(set balancer.ConnSet, addr resolver.Address, index int) balancer.Conn {
